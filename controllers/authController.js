...
exports.register = async (req, res) => {
    ...
    try {
        ...
        if (userExists.rows.length > 0) {
            return res.status(400).json({ error: 'Uživatel s tímto emailem už existuje' });
        }
        ...
        await db.query(
            'INSERT INTO "UserProgress" ("UserId", "XP", "Level", "WeeklyStreak") VALUES ($1, 0, 1, 0)',
            [newUser.rows[0].Id]
        );

        res.status(201).json({ message: 'Registrace úspěšná!', user: newUser.rows[0] });
    } catch (err) {
        console.error(err);
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        res.json({ message: 'Přihlášení úspěšné!', token, user: { id: user.rows[0].Id, userName: user.rows[0].UserName } });
    } catch (err) {
        console.error(err);
